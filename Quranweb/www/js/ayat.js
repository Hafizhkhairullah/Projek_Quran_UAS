                              ...
                              <div class="flex justify-between items-center">
                                  <div>
                                      ...
                                      <select id="ayat" class="mt-1 block w-full bg-gray-100 border border-gray-300 rounded-md py-2 px-3 text-gray-700">
                                          ${Array.from({ length: surahData.jumlah_ayat }, (_, i) => `<option value="${i + 1}">${i + 1}</option>`).join("")}
                                      </select>
                                  </div>
                                  <div>
      ...
      fetch(`http://127.0.0.1:8000/api/surah/${nomorSurah}`)
          ...
          .then((result) => {
              ...
              data.forEach((ayat) => {
                  ...
                  ayatElement.setAttribute("data-nomor", ayat.nomor_ayat);
                  ayatElement.innerHTML = `
                      ...
                      <div class="content">
                          ...
                          <div class="flex space-x-4 mt-4">
                              ...
                              <div class="bg-black rounded-full px-4 py-2 flex items-center space-x-2 tafsir-toggle" data-surah="${nomorSurah}" data-ayat="${ayat.nomor_ayat}">
                                  <i class="fas fa-file-alt text-white"></i>
                                  <span class="text-white">Tafsir</span>
                              </div>
                          </div>
                          <div class="tafsir-content hidden p-4 mt-4 bg-gray-200 dark:bg-gray-800 rounded-lg">
                              <p class="teks-tafsir"></p>
                          </div>
                      </div>
                  `;
                  ayatList.appendChild(ayatElement);
                  
                  const tafsirToggle = ayatElement.querySelector(".tafsir-toggle");
                  tafsirToggle.addEventListener("click", () => {
                    const nomorAyat = tafsirToggle.getAttribute("data-ayat");
                    const tafsirContent = ayatElement.querySelector(".tafsir-content");
                    const teksTafsir = tafsirContent.querySelector(".teks-tafsir");
                    if (tafsirContent.classList.contains("hidden")) {
                      fetch(`http://localhost:8000/api/tafsir/${nomorSurah}/${nomorAyat}`)
                       .then((response) => response.json())
                       .then((result) => {
                        const tafsirData = result.data[0];
                        teksTafsir.innerHTML = tafsirData.teks_tafsir;
